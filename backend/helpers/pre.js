...
module.exports.companies = {
  method: async (request, h) => {
    ...
    let companies = await request.server.methods.deck.getCompanies(
      edition,
      user,
      token,
    );
    ...
    for (let company of companies) {
      let link = await request.server.methods.link.find({
...
module.exports.company = {
  method: async (request, h) => {
    ...
    const user = request.auth.credentials.user;
    const token = request.auth.credentials.token;
    let company = await request.server.methods.deck.getCompany(
      companyId,
      user,
      token,
    );
...
module.exports.member = {
  method: async (request, h) => {
    ...
    const member = await request.server.methods.deck.getMember(
      memberId,
    );
    return member;
  },
