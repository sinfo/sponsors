...
const URL = `${config.DECK.HOST}:${config.DECK.PORT}`
...
async function getLatestEdition () {
  const events = await request({
    method: 'GET',
    uri: `${URL}/events?current=true`,
    json: true
  })

  let latestEvent = events.length > 0 ? events[0] : null
  let latestEventDate = events.length > 0 ? new Date(events[0].begin).getTime() : null

  events.forEach(event => {
    let thisDate = new Date(event.begin).getTime()
